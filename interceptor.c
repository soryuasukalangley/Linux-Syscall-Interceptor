...
MODULE_DESCRIPTION("ass1");
MODULE_AUTHOR("Xinhao Hou");
MODULE_AUTHOR("Tianchen Xu");
MODULE_LICENSE("GPL");
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if(table[reg.ax].monitored == 1){
		int stats = check_pid_monitored(reg.ax, current->pid);
		if (stats == 1){
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	// input valid?
	if(syscall == MY_CUSTOM_SYSCALL || syscall < 0 || syscall > NR_syscalls -1){
		return -EINVAL;
	}
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		set_addr_rw((unsigned long) sys_call_table);
 		...
 		table[syscall].f = (asmlinkage long (*)(struct pt_regs)) sys_call_table[syscall];
		...
		//pid is not 0 and invalid
		if(pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)
			return -EINVAL;
		
		//not root
		if(current_uid() != 0)
			if(pid == 0 || 0 != check_pids_same_owner(pid, current->pid))
				return -EPERM;
		...
		//monitor all?
		if(pid == 0){
			//clear current list(not useful) and set monitored to 2 so all pid monitoring
			destroy_list(syscall);
		...
		if((check_pid_monitored(syscall, pid) == 0 || !table[syscall].intercepted) && table[syscall].monitored != 2){
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}

		if(pid == 0){
			//clear all monitored pid
			destroy_list(syscall);
